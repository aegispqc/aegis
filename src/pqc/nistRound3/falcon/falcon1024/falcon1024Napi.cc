...
#define FALCON1024_CRYPTO_PK_SIZE           CRYPTO_PUBLICKEYBYTES
...
Napi::Boolean NapiSignBySeed( const Napi::CallbackInfo& info )
{
    ...
    unsigned char sk[FALCON1024_CRYPTO_SK_SIZE];
    const int gensk = falcon_seed_to_sk( sk, seed );
    if( gensk != 0 ){
        return Napi::Boolean::New( env, false );
    }
...
{
    return Napi::Number::New(info.Env(), FALCON1024_CRYPTO_SK_SIZE);
}

Napi::Number getPkLength( const Napi::CallbackInfo& info ) 
{
    return Napi::Number::New(info.Env(), FALCON1024_CRYPTO_PK_SIZE);
}

Napi::Number getSignLength( const Napi::CallbackInfo& info ) 
{
    return Napi::Number::New(info.Env(), FALCON1024_CRYPTO_SIGN_SIZE);
}

Napi::Number getNonceLength( const Napi::CallbackInfo& info ) 
{
    return Napi::Number::New(info.Env(), FALCON1024_CRYPTO_NONCE_LEN);
}

Napi::Number getSeedLength( const Napi::CallbackInfo& info ) 
{
    return Napi::Number::New(info.Env(), FALCON1024_CRYPTO_SEED_LEN);
}
...
NODE_API_MODULE( crypto_falcon, Init );